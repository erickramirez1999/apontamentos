...
from src.utils.estilo import card_kpi, COR_VERDE
...
def renderizar(usuario):
    ...
    _renderizar_lista(tipo="EXCLUSAO", usuario=usuario)
...
def _renderizar_lista(tipo: str, usuario):
    ...
    if not eventos:
        st.info(
            "📭 **Nenhum arquivo de exclusão carregado ainda.**\n\n"
    ...
    with c1:
        st.markdown(card_kpi(
            "Arquivos", f"{total_arquivos:,}", "carregados", COR_VERDE, "📤"
        ), unsafe_allow_html=True)
    with c2:
        st.markdown(card_kpi(
            "Títulos", f"{total_titulos:,}", "registrados", COR_VERDE, "📋"
        ), unsafe_allow_html=True)
                ...
                with col_btn:
                    if permite_excluir:
                        confirmar_exclusao_com_senha(
                            ...
                            chave=f"del_serasa_exc_{e['id']}",
                            descricao_item=f"Exclusão #{e['sequencial']}",
